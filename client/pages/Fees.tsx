...
type LedgerItem = { id: string; date: string; amount: number; description: string; type: "invoice" | "payment" };
...
function saveFees(v: FeesState) { localStorage.setItem("fees", JSON.stringify(v)); }
...
export default function Fees() {
  ...
  const filtered = useMemo(() => {
    return students.filter(s => `${s.firstName} ${s.lastName}`.toLowerCase().includes(q.toLowerCase()));
  }, [students, q]);

  function addItem(studentId: string) {
    const entry: LedgerItem = { id: uid(), date, amount: Number(amount), description, type: mode };
              ...
              {filtered.map(s => {
                ...
                return (
                  <tr key={s.id} className="border-t">
                    ...
                    <td className={`py-2 font-medium ${balance>0?"text-red-600":"text-green-700"}`}>P {balance.toLocaleString()}</td>
                    <td className="py-2">
                      <button onClick={()=>setSelected(s.id)} className="px-3 py-1 rounded-md border">Open Ledger</button>
                    </td>
                  </tr>
                );
              })}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
                    <div className="rounded-lg bg-blue-50 p-3">
                      ...
                      <div className="text-lg font-bold">P {invoices.toLocaleString()}</div>
                    </div>
                    <div className="rounded-lg bg-green-50 p-3">
                      <div className="text-xs text-muted-foreground">Paid</div>
                      <div className="text-lg font-bold text-green-700">P {payments.toLocaleString()}</div>
                    </div>
                    <div className="rounded-lg bg-white border p-3">
                      <div className="text-xs text-muted-foreground">Balance</div>
                      <div className={`text-lg font-bold ${balance>0?"text-red-600":"text-green-700"}`}>P {balance.toLocaleString()}</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4">
                    ...
                    <form
                      ...
                      onSubmit={(e)=>{ e.preventDefault(); addItem(selected!); }}
                    >
                      <div>
                        <label className="block text-sm mb-1">Date</label>
                        <input type="date" value={date} onChange={(e)=>setDate(e.target.value)} className="w-full rounded-md border px-3 py-2" required />
                      </div>
                      <div>
                        <label className="block text-sm mb-1">Type</label>
                        <select value={mode} onChange={(e)=>setMode(e.target.value as any)} className="w-full rounded-md border px-3 py-2">
                      ...
                      <div>
                        ...
                        <input type="number" min={0} step="0.01" value={amount} onChange={(e)=>setAmount(Number(e.target.value))} className="w-full rounded-md border px-3 py-2" required />
                      </div>
                      <div className="md:col-span-2">
                        <label className="block text-sm mb-1">Description</label>
                        <input value={description} onChange={(e)=>setDescription(e.target.value)} className="w-full rounded-md border px-3 py-2" placeholder="e.g., Term 3 Fees" />
                      </div>
                      ...
                      <tbody>
                        {items
                          ...
                          <tr key={i.id} className="border-t">
                            ...
                            <td className="py-2">{i.type === "invoice"? "Invoice":"Payment"}</td>
                            <td className="py-2">P {i.amount.toLocaleString()}</td>
                            <td className="py-2">{i.description}</td>
                            <td className="py-2">
                              <button
                                onClick={()=>{
                                  const next = { ...fees, [selected!]: { items: (fees[selected!]?.items || []).filter(x => x.id !== i.id) } };
                                  setFees(next);
                                }}
