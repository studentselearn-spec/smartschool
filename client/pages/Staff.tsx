...
function loadPerformance(): Record<
  string,
  { rating: number; notes?: string; updatedAt: string }
> {
...
function savePerformance(
  v: Record<string, { rating: number; notes?: string; updatedAt: string }>,
) {
  localStorage.setItem("staffPerformance", JSON.stringify(v));
}

export default function StaffPage() {
  const [tab, setTab] = useState<"records" | "attendance" | "performance">(
    "records",
  );
  ...
  const [attendance, setAttendance] = useState<AttendanceEntry[]>(() =>
    loadStaffAttendance(new Date().toISOString().slice(0, 10)),
  );
  ...
  useEffect(() => {
    setAttendance(loadStaffAttendance(date));
  }, [date]);
  useEffect(() => {
    saveStaffAttendance(date, attendance);
  }, [date, attendance]);
  useEffect(() => {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1 rounded-md border text-sm ${tab === "records" ? "bg-blue-600 text-white" : "bg-white"}`}
            >
              Records
            </button>
            <button
              onClick={() => setTab("attendance")}
              className={`px-3 py-1 rounded-md border text-sm ${tab === "attendance" ? "bg-blue-600 text-white" : "bg-white"}`}
            >
              Attendance
            </button>
            <button
              onClick={() => setTab("performance")}
              className={`px-3 py-1 rounded-md border text-sm ${tab === "performance" ? "bg-blue-600 text-white" : "bg-white"}`}
            >
              Performance
            </button>
          </div>
          ...
          <div className="rounded-xl border bg-white p-4 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              <div className="text-sm text-muted-foreground">
                Add and manage teachers and staff
              </div>
              <button
                onClick={openNew}
          ...
          <div className="rounded-xl border bg-white p-4 shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {staff.map((s) => {
                    const rec = attendance.find((a) => a.staffId === s.id) || {
                      staffId: s.id,
                      present: true,
                    };
                    return (
            ...
            <div className="mt-3 text-sm text-muted-foreground">
              Present: {presentCount} / {staff.length}
            </div>
          </div>
        )}
        ...
        {tab === "performance" && (
          <div className="rounded-xl border bg-white p-4 shadow-sm">
            <div className="text-sm text-muted-foreground mb-3">
              Update performance ratings (1-5) and notes
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                    ...
                    return (
                      <tr key={s.id} className="border-t">
                        ...
                        <td className="py-2 text-xs text-muted-foreground">
                          {new Date(perf.updatedAt).toLocaleString()}
                        </td>
                      </tr>
                    );
                  })}
