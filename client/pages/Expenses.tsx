...
function saveExpenses(items: Expense[]) { localStorage.setItem("expenses", JSON.stringify(items)); }
...
export default function ExpensesPage() {
  ...
  const [form, setForm] = useState<Expense>({ id: "", date: new Date().toISOString().slice(0,10), category: "Salaries", amount: 0, notes: "" });
    ...
    <Layout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={()=>{ setForm({ id: "", date: new Date().toISOString().slice(0,10), category: "Salaries", amount: 0, notes: "" }); setShowForm(true); }} className="px-3 py-2 rounded-md bg-blue-600 text-white text-sm">Add Expense</button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <tbody>
                {items.map(e => (
                  <tr key={e.id} className="border-t">
                    ...
                    <td className="py-2">
                      <button onClick={()=> setItems(items.filter(x => x.id !== e.id))} className="px-2 py-1 rounded-md border text-red-600">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="rounded-xl border bg-white p-4 shadow-sm">
            ...
            <div className="mt-2 text-2xl font-bold">P {total.toLocaleString()}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
            ...
            <form
              ...
              onSubmit={(e)=>{
                e.preventDefault();
                setItems([...items, { ...form, id: uid(), amount: Number(form.amount) }]);
                setShowForm(false);
              }}
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" value={form.date} onChange={(e)=>setForm({ ...form, date: e.target.value })} required className="w-full rounded-md border px-3 py-2" />
                </div>
                <div>
              ...
              <div>
                ...
                <input type="number" min={0} step="0.01" value={form.amount} onChange={(e)=>setForm({ ...form, amount: Number(e.target.value) })} required className="w-full rounded-md border px-3 py-2" />
              </div>
              <div>
                <label className="block text-sm mb-1">Notes</label>
                <input value={form.notes || ""} onChange={(e)=>setForm({ ...form, notes: e.target.value })} className="w-full rounded-md border px-3 py-2" placeholder="Optional" />
              </div>
